...
conn.execute('''CREATE TABLE IF NOT EXISTS EXHIBITION
    ...
    PRIMARY KEY (ID),
    CONSTRAINT PERIOD_TRUE CHECK (Periodical == FALSE OR Date_Export IS NOT NULL));''')
...
conn.execute('''CREATE TABLE IF NOT EXISTS ARTWORK
    ...
    PRIMARY KEY (ID),
    FOREIGN KEY(ID_Exhibition) REFERENCES EXHIBITION(ID) ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY(ID_Hall) REFERENCES HALL(ID) ON DELETE RESTRICT ON UPDATE CASCADE,
...
conn.execute('''CREATE TABLE IF NOT EXISTS BORROWS_FROM
    ...
    FOREIGN KEY(ID_Borrowing) REFERENCES BORROWING(ID) ON DELETE CASCADE ON UPDATE CASCADE);''')
...
conn.execute('''CREATE TABLE IF NOT EXISTS MAINTENANCE
    ...
    FOREIGN KEY(ID_Artwork) REFERENCES ARTWORK(ID) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(VAT_Supervisor) REFERENCES EMPLOYEE_MAINTENANCE(VAT) ON DELETE RESTRICT ON UPDATE CASCADE);''')

conn.execute('''CREATE TABLE IF NOT EXISTS CONDUCTS
    ( ID_Artwork INTEGER NOT NULL,
    VAT INTEGER NOT NULL,
    PRIMARY KEY (ID_Artwork,VAT),
    FOREIGN KEY(ID_Artwork) REFERENCES MAINTENANCE(ID_Artwork) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY(VAT) REFERENCES EMPLOYEE_MAINTENANCE(VAT) ON DELETE RESTRICT ON UPDATE CASCADE);''')
